...
--====================================================================================
--  /* 获取 Accept 预投递数量 */
function BaseAgent:取预投递数量()
    return self._hp:GetAcceptSocketCount()
end

--  /* 获取通信数据缓冲区大小 */
function BaseAgent:取缓冲区大小()
    return self._hp:GetSocketBufferSize()
end

--  /* 获取监听 Socket 的等候队列大小 */
function BaseAgent:取等候队列大小()
    return self._hp:GetSocketListenQueue()
end

--  /* 获取正常心跳包间隔 */
function BaseAgent:取正常心跳间隔()
    return self._hp:GetKeepAliveTime()
end

--  /* 获取异常心跳包间隔 */
function BaseAgent:取异常心跳间隔()
    return self._hp:GetKeepAliveInterval()
end

--====================================================================================
function BaseAgent:启动(ip, async)
    return self._hp:Start(ip, async)
end

function BaseAgent:连接(ip, port)
    return self._hp:Connect(ip, port)
end

--获取某个连接的远程主机信息
function BaseAgent:取远程信息(id)
    return self._hp:GetRemoteHost(id)
end

function BaseAgent:停止()
    self._hp:Stop()
    return self
end

function BaseAgent:发送(id, pBuffer, iOffset)
    return self._hp:Send(id, pBuffer, iOffset)
end

function BaseAgent:暂停接收(id, bPause)
    return self._hp:PauseReceive(id, bPause)
end

-- 连接 ID-- 是否强制断开连接
function BaseAgent:断开(id, bForce)
    return self._hp:Disconnect(id, bForce)
end

-- 时长（毫秒）-- 是否强制断开连接
function BaseAgent:断开超时(id, dwPeriod, bForce)
    return self._hp:DisconnectLongConnections(id, dwPeriod, bForce)
end

--描述：断开超过指定时长的静默连接
-- 时长（毫秒）-- 是否强制断开连接
function BaseAgent:断开静默(id, dwPeriod, bForce)
    return self._hp:DisconnectSilenceConnections(id, dwPeriod, bForce)
end

--描述：等待通信组件停止运行
function BaseAgent:等待(dwMilliseconds)
    return self._hp:Wait(dwMilliseconds)
end

--====================================================================================
function BaseAgent:是否启动()
    return self._hp:HasStarted()
end

function BaseAgent:取状态()
    return HPS[self._hp:GetState()]
end

--  /* 获取连接数 */
function BaseAgent:取连接数()
    return self._hp:GetConnectionCount()
end

--/* 获取所有连接的 CONNID */
function BaseAgent:取所有连接ID()
    return self._hp:GetAllConnectionIDs()
end

--  /* 获取某个连接时长（毫秒） */
function BaseAgent:取连接时长(id)
    return self._hp:GetConnectPeriod(id)
end

--  /* 获取某个连接静默时间（毫秒） */
function BaseAgent:取静默时长(id)
    return self._hp:GetSilencePeriod(id)
end

--  /* 获取某个连接的本地地址信息 */
function BaseAgent:取本地地址(id)
    return self._hp:GetLocalAddress(id)
end

--/* 获取某个连接的远程地址信息 */
function BaseAgent:取远程地址(id)
    return self._hp:GetRemoteAddress(id)
end
...
-- SE_DATA_PROC               // 数据处理错误
-- SE_DATA_SEND               // 数据发送失败
function BaseAgent:取错误代码()
    return self._hp:GetLastError()
end

--  /* 获取最近一次失败操作的错误描述 */
function BaseAgent:取错误描述()
    return self._hp:GetLastErrorDesc()
end

--/* 获取连接中未发出数据的长度 */
function BaseAgent:取未发出数据长度(id)
    return self._hp:GetPendingDataLength(id)
end

--/* 获取连接的数据接收状态 */
function BaseAgent:是否暂停接收(id)
    return self._hp:IsPauseReceive(id)
end

function BaseAgent:是否已连接(id)
    return self._hp:IsConnected(id)
end
...
function BaseAgent:置地址重用策略(enReusePolicy)
...
--  SP_SAFE     // 安全模式
--  SP_DIRECT   // 直接模式
function BaseAgent:置数据发送策略(enSendPolicy)
...
function BaseAgent:置静默时间(bMarkSilence)
