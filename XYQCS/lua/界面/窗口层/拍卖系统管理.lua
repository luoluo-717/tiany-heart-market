...
--]]
local 商会物品 = 多开数据[多开数据.当前].__UI界面.窗口层:创建我的窗口("拍卖系统管理",0,0,579,381)
local 当前 = 多开数据.当前
function 商会物品:初始化()
    __置窗口坐标(self)
    local nsf = require('SDL.图像')(579,381)
    if nsf:渲染开始() then
        置窗口背景("拍卖系统管理",0,12,572,367,true):显示(0,0)
...
function 道具网格:置物品(data,bh)
    if not bh then
        for i = 1,#self.子控件 do
            ...
            lssj:置物品(data[i],"白格子","拍卖道具")
            self.子控件[i]:置精灵(lssj)
        end
    else
        local lssj = __物品格子.创建(当前)
        lssj:置物品(data,"白格子","拍卖道具")
