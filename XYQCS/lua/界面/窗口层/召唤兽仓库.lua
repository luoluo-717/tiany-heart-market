...
--]]
local 召唤兽仓库 = 多开数据[多开数据.当前].__UI界面.窗口层:创建我的窗口("召唤兽仓库",0,0,777,488)
local 当前 = 多开数据.当前
function 召唤兽仓库:初始化()
    self:置精灵(置窗口背景("召唤兽仓库",0,12,723,476))
end
...
function 召唤兽仓库:打开(data)
    ...
    self:置可见(true)
    self.页数 = 1
    self.总页= data.召唤兽仓库总数
    self.数据 = data.召唤兽仓库数据
    self.召唤兽控件:重置(self.数据)
    self.召唤兽控件:置可见(true)
end

for i, v in ipairs {
    {name = '增加', x = 160 ,y = 410 , tcp = __res:getPNGCC(3,244,563,78,34,true):拉伸(46,46) ,font = '+'},
...
function 召唤兽仓库:刷新(data)
    self.总页= data.召唤兽仓库总数 or self.总页
    self.数据 = data.召唤兽仓库数据
    self.召唤兽控件:重置(self.数据)
end
...
function 召唤兽控件:重置(data)
    ...
    召唤兽仓库.图像 = nsf:到精灵()
    召唤兽仓库.召唤兽数据 = 多开数据[当前 or self and self.当前].角色信息.宝宝列表
    召唤兽仓库.仓库召唤兽数据 = data
    self.召唤兽列表:重置(召唤兽仓库.召唤兽数据)
...
for i, v in ipairs {
    ...
    function 临时函数:左键弹起(x, y)
        ...
        elseif v.name == '查看2' then
            if 召唤兽仓库.道具选中 and 召唤兽仓库.召唤兽数据[召唤兽仓库.道具选中] then 
                多开数据[当前].__UI界面.窗口层.召唤兽查看:打开(召唤兽仓库.召唤兽数据[召唤兽仓库.道具选中])
            else
...
for i, v in ipairs {
    {name = '存入', x = 331 ,y = 160 , tcp = __res:getPNGCC(3,335,561,96,35):拉伸(60,35)},
    {name = '取出', x = 331 ,y = 335 , tcp = __res:getPNGCC(3,436,561,96,34):拉伸(60,35)},
} do
    local  临时函数 = 召唤兽控件:创建我的按钮(v.tcp,v.name,v.x,v.y)
    function 临时函数:左键弹起(x, y)
        if v.name == '存入' and 召唤兽仓库.道具选中 and 召唤兽仓库.召唤兽数据[召唤兽仓库.道具选中] then
            发送数据(当前 or self and self.当前,6802,{类型="存",认证码=召唤兽仓库.召唤兽数据[召唤兽仓库.道具选中].认证码,页数=召唤兽仓库.页数})
        elseif v.name == '取出' and 召唤兽仓库.仓库选中 and 召唤兽仓库.仓库召唤兽数据[召唤兽仓库.仓库选中] then
            发送数据(当前 or self and self.当前,6802,{类型="取",认证码=召唤兽仓库.仓库召唤兽数据[召唤兽仓库.仓库选中].认证码,页数=召唤兽仓库.页数})
        end
    end
end
