...
--]]
local 声望商店 = 多开数据[多开数据.当前].__UI界面.窗口层:创建窗口("声望商店",0,0,377,510)
local 当前 = 多开数据.当前
local lsb = {"单价","总价","数量","声望"}
local lsb2 = {"单价","总价","声望"}
...
function 声望商店:初始化()
    ...
    self.总价 = 0
    self.现金 = 0
    self.标题宽度 = 字体20:取宽度("声望商店")
    self.标题 = "声望商店"
end

function 声望商店:显示(x,y)
    if self.图像 then
        self.图像:显示(x,y)
    end
...
local 关闭 = 声望商店:创建我的按钮(__res:getPNGCC(1,401,0,46,46),"关闭",327,0)
...
function 声望商店:重置(数据)
    ...
    if nsf:渲染开始() then
        ...
        字体18:取图像(self.声望):显示(250,422)
        nsf:渲染结束()
    end
    self.图像 = nsf:到精灵()
end

local 道具网格 = 声望商店:创建网格("道具网格",23,57,326,305)
...
function 道具网格:左键弹起(x, y, a, b, msg)
    if self.子控件[a] and self.子控件[a]._spr and self.子控件[a]._spr.物品 then
        ...
        if 声望商店.选中道具 and self.子控件[声望商店.选中道具]._spr.物品 then
            self.子控件[声望商店.选中道具]._spr.确定 = nil
        end
        声望商店.选中道具 = a
        self.子控件[a]._spr.确定 = true
        self.子控件[a]._spr:详情打开(170,86,w,h,"选择",a)
        声望商店:重置(声望商店.临时道具[声望商店.选中道具])
    end
end
...
local 数量输入 = 声望商店:创建输入("数量输入",285,388,50,18)
function 数量输入:初始化()
    self:取光标精灵()
    self:置限制字数(2)
...
for i=1,#lsan do
    ...
    local 临时函数 = 声望商店:创建我的按钮(__res:getPNGCC(3,2,507,124,41):拉伸(123,41),lsan[i].."按钮",35+pyx,450+((i-1)*30)+pyy,lsan[i])
    ...
    function 临时函数:左键弹起(x,y,msg)
        ...
        elseif lsan[i] == "购买" then
            if 声望商店.选中道具 and 道具网格.子控件[声望商店.选中道具]._spr.物品 then
                if 声望商店.数量输入:取数值() then
                    
                    发送数据(当前 or self and self.当前,1503.5,{数量=声望商店.数量输入:取数值(),商品=声望商店.道具网格.子控件[声望商店.选中道具]._spr.物品.原始商品,类型=声望商店.类型,商店名称=声望商店.标题})
