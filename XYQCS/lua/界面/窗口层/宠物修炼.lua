--[[
    @Author       : GGELUA
    @Date         : 2022-05-09 10:59:42
Last Modified by: GGELUA
Last Modified time: 2023-11-07 15:20:25
--]]
local 宠物修炼 = 多开数据[多开数据.当前].__UI界面.窗口层:创建我的窗口("宠物修炼",0,0,485,365)
local 当前 = 多开数据.当前
...
function 宠物修炼:初始化()
    ...
    if nsf:渲染开始() then
        置窗口背景("宠物修炼",0,12,477,356,true):显示(0,0)
...
function 宠物修炼:打开(data)
    self:置可见(true)
    self:重置(data)
    self.选中 = nil
end

function 宠物修炼:重置(data)
    self.数据 = data
    self.免资材=data.免资材
    ...
    self.图像 = nsf:到精灵()
    self.图像:置中心(0,-227)
    self.修炼列表:重置(self.数据.宠物修 or self.数据.修炼)
end

local 关闭 = 宠物修炼:创建我的按钮(__res:getPNGCC(1,401,0,46,46),"关闭",435,0)

function 关闭:左键弹起(x,y,msg)
    宠物修炼:置可见(false)
end

local 修炼列表 = 宠物修炼:创建列表("修炼列表",22,50,436,161)
...
local lsb = {"攻击控制力","防御控制力","法术控制力","抗法控制力"}
...
local 修炼 = 宠物修炼:创建我的按钮(__res:getPNGCC(3,2,507,124,41,true):拉伸(123,41),"修炼",200,305,"修炼")
function 修炼:左键弹起(x,y,msg)
    if 宠物修炼.选中 then
        发送数据(当前 or self and self.当前,44,{类型="宝宝修炼",免资材=宠物修炼.免资材,修炼项目=lsb[宠物修炼.选中]})
    else
