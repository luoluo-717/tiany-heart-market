local 点卡寄售 = 多开数据[多开数据.当前].__UI界面.窗口层:创建我的窗口("点卡寄售",0,0,410,410)
local 当前 = 多开数据.当前
...
function 点卡寄售:初始化()
    ...
    if nsf:渲染开始() then
        置窗口背景("点卡寄售",0,12,400,400,true):显示(0,0)
        nsf:渲染结束()
    end
    self:置精灵(nsf:到精灵())
end

function 点卡寄售:打开(data)
    self:置可见(true)
    self.数据 = data
    self.点卡数据 = {}
    self.我得点卡 = {}
    ...
    for i=1,#self.数据.商品 do
        ...
        self.点卡数据[#self.点卡数据+1]={真实编号=i,点数=ls[3],单价=ls[4],时间=ls[5],识别码=ls[6]}
        if ls[2]+0 == 多开数据[当前 or self and self.当前].角色信息.数字id then
            self.我得点卡[#self.我得点卡+1]={真实编号=i,点数=ls[3],单价=ls[4],时间=ls[5],识别码=ls[6]}
            self.已寄售点数=self.已寄售点数+self.我得点卡[#self.我得点卡].点数
        end
    end
    if #self.我得点卡 >1 then
        table.sort(self.我得点卡,function(a,b) return a.时间<b.时间 end)
    end
    if #self.点卡数据 >1 then
        table.sort(self.点卡数据,function(a,b) return a.单价<b.单价 end)
    end
    self.购买点卡:置选中(true)
    self.购买:置可见(true)
    self.购买:重置()
...
local 购买 = 点卡寄售:创建控件('购买',20,60,400, 400)
...
function 购买:重置()
    ...
    if nsf:渲染开始() then
        ...
        字体16:取图像('携带银子：'..点卡寄售.数据.银子):显示(240,335)
    end
    点卡寄售.图像 = nsf:到精灵()
    self.购买列表:重置(点卡寄售.点卡数据)
end
...
function 购买:显示(x,y)
    if self.是否可见 and 购买.选中 and tonumber(self.购买点数:取数值()) ~= 0 and tonumber(self.购买点数:取数值()) ~= nil then
        字体16:显示(x+300,y+250,self.购买点数:取数值()*((点卡寄售.点卡数据[购买.选中] or {}).单价 or 0))
        字体16:置颜色(__取颜色(黑色))
        字体16:显示(x+95,y+275,(点卡寄售.点卡数据[购买.选中] or {}).单价 or 0)
    end
end
...
function 确定购买:左键弹起(消息,x,y)
    ...
    local 编号 = 点卡寄售.点卡数据[购买.选中].真实编号
    发送数据(当前 or self and self.当前,65,{编号=编号,数量=tonumber(购买.购买点数:取数值()),识别码=点卡寄售.点卡数据[购买.选中].识别码})
end

local 出售 = 点卡寄售:创建控件('出售',20,60,400, 400)
...
function 出售:重置()
    ...
    if nsf:渲染开始() then
        ...
        字体16:取图像('可寄售点数:'..点卡寄售.数据.点卡):显示(30,310)
        字体16:取图像('已寄售点数:'..点卡寄售.已寄售点数):显示(30,335)
        local lssj = 取输入背景(0, 0, 120, 23)
...
function 寄售:左键弹起(消息,x,y)
    ...
    发送数据(当前 or self and self.当前,63,{数量=tonumber(出售.寄售点数:取数值()),单价=tonumber(出售.寄售单价:取数值())})
end
...
for i, v in ipairs {
    {name = '购买点卡', x = 90 ,y = 45 , tcp = __res:getPNGCC(3,511,11,117,43,true):拉伸(120,25), tcp2 = __res:getPNGCC(3,390,12,118,43,true):拉伸(120,25),font="购买点卡"},
    {name = '出售点卡', x = 90+90+35 ,y = 45 , tcp = __res:getPNGCC(3,511,11,117,43,true):拉伸(120,25), tcp2 = __res:getPNGCC(3,390,12,118,43,true):拉伸(120,25),font="出售点卡"},
} do
    local 临时函数 = 点卡寄售:创建我的单选按钮(v.tcp,v.tcp2,v.name,v.x,v.y,v.font)
...
local 关闭 = 点卡寄售:创建我的按钮(__res:getPNGCC(1,401,0,46,46):置拉伸(30,30),"关闭",370,12)
