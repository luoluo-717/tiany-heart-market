local 锦衣焕彩继承 = 多开数据[多开数据.当前].__UI界面.窗口层:创建我的窗口("锦衣焕彩继承",160 + abbr.py.x,50 + abbr.py.y,750,450)
local 当前 =  多开数据.当前
function 锦衣焕彩继承:初始化()
...
function 锦衣焕彩继承:重置()
  ...
  锦衣焕彩继承["图像"] = nsf["到精灵"](nsf)
  锦衣焕彩继承["图像"]["置中心"](锦衣焕彩继承["图像"], 0, -398)
  self.道具网格["置物品"](self.道具网格, 多开数据[多开数据.当前].__主控["道具列表"])
  self.材料网格["置物品"](self.材料网格, {})
end
function 锦衣焕彩继承:刷新道具(data)
  self.道具网格:置物品(data, a)
end

local 关闭 = 锦衣焕彩继承["创建我的按钮"](锦衣焕彩继承, __res:getPNGCC(1, 401, 0, 46, 46), "关闭", 367+260, 0)
function 关闭:左键弹起(x, y, msg)
  锦衣焕彩继承["置可见"](锦衣焕彩继承, false)
end


local 道具网格 = 锦衣焕彩继承["创建网格"](锦衣焕彩继承, "道具网格", 43+246, 128-28, 318, 252)
function 道具网格:初始化()
  self:创建格子(55, 55, 10, 10, 4, 5)
end
function 道具网格:左键弹起(x, y, a, b, msg)
  if self.子控件[a]._spr["物品"] then
    for i = 1, #锦衣焕彩继承["材料网格"]["子控件"] do
      if not 锦衣焕彩继承["材料网格"]["子控件"][i]._spr["物品"] then
        self.子控件[a]._spr["详情打开"](self.子控件[a]._spr, 170, 86, w, h, "选择", a)
        锦衣焕彩继承["材料网格"]["置物品"](锦衣焕彩继承["材料网格"], self.子控件[a]._spr["物品"], i)
        锦衣焕彩继承["材料网格"]["子控件"][i]._spr["物品"]["原始编号"] = a
        self:置物品(nil, a)
        break
      end
    end
...
local 材料网格 = 锦衣焕彩继承["创建网格"](锦衣焕彩继承, "材料网格", 180-120, 130, 55, 180)
function 材料网格:初始化()
...
function 材料网格:左键弹起(x, y, a, b, msg)
  if self.子控件[a]._spr["物品"] then
    -- self.子控件[a]._spr["详情打开"](self.子控件[a]._spr, 170, 86, w, h, "选择", a)
    锦衣焕彩继承["道具网格"]["置物品"](锦衣焕彩继承["道具网格"], self.子控件[a]._spr["物品"], self.子控件[a]._spr["物品"]["原始编号"])
    self:置物品(nil, a)
  end
end
...
for i, v in ipairs({
  ...
  local 临时函数 = 锦衣焕彩继承["创建我的按钮"](锦衣焕彩继承, v.tcp, v.name, v.x, v.y, v.font)
 function  临时函数:左键弹起(x, y)
    if v.name == "继承" then
          if 锦衣焕彩继承["材料网格"]["子控件"][1]._spr["物品"] and 锦衣焕彩继承["材料网格"]["子控件"][2]._spr["物品"] then
          发送数据(当前,3739.2, {
            格子1 = 锦衣焕彩继承["材料网格"]["子控件"][1]._spr["物品"]["原始编号"],
            格子2 = 锦衣焕彩继承["材料网格"]["子控件"][2]._spr["物品"]["原始编号"]
