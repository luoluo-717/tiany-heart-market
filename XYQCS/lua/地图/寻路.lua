...
-- @Last Modified time: 2021-11-25 08:59:14
...
function 地图寻路:寻路(xy,txy,f)
    local x,y   = _坐标到格子(xy)
    local tx,ty = _坐标到格子(txy)
    if f ~= 1 and self:判断直线障碍(xy,txy) then
        ...
        self._p = {_坐标:pack(txy.x/20,txy.y/20):floor()}
        
    end
    --return self._p
    if #self._p == 1 then
        return {{x=self._p[1].x,y=self._p[1].y}}
    else
        return GGEF.tablecopy(self._p)
    end
end
